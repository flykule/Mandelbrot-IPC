...
MandelbrotWidget::MandelbrotWidget(QWidget *parent)
    : QWidget(parent), mMandelbrotCalculator(), mThreadCalculator(this),
      mScaleFactor(DEFAULT_SCALE), mLastMouseMovePosition(),
      mMoveOffset(DEFAULT_OFFSET_X, DEFAULT_OFFSET_Y), mAreSize(),
      mIterationMax(ITERATION_MAX) {
  mMandelbrotCalculator.moveToThread(&mThreadCalculator);
  connect(this, &MandelbrotWidget::requestPicture, &mMandelbrotCalculator,
...
void MandelbrotWidget::processJobResults(QList<JobResult> jobResults) {
  ...
  for (JobResult &jobResult : jobResults) {
    ...
    for (int x = 0; x < mAreSize.width(); ++x) {
...
void MandelbrotWidget::mouseMoveEvent(QMouseEvent *event) {
  if (event->buttons() & Qt::LeftButton) {
    ...
    emit requestPicture(mAreSize, mMoveOffset, mScaleFactor, mIterationMax);
  }
}
...
void MandelbrotWidget::paintEvent(QPaintEvent *event) {
  ...
  painter.drawText(
      10, 20,
      QString("Size: %1 x %2").arg(mImage->width()).arg(mImage->height()));

  painter.drawText(
      10, 35,
      QString("Offset: %1 x %2").arg(mMoveOffset.x()).arg(mMoveOffset.y()));

  painter.drawText(10, 50, QString("Scale: %1").arg(mScaleFactor));
...
void MandelbrotWidget::resizeEvent(QResizeEvent *event) {
  ...
  mImage = std::make_unique<QImage>(mAreSize, QImage::Format_RGB32);
